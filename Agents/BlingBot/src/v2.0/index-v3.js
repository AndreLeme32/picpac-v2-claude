...
async function routes(fastify, options) {
  fastify.post('/receberPropostaDoDecisor', async (request, reply) => {
    try {
      ...
      if (!proposta || !proposta.cliente || !proposta.itens) {
        fastify.log.warn('Proposta invÃ¡lida recebida no BlingBot');
        return reply.status(400).send({
          error: 'Proposta invÃ¡lida - faltam dados obrigatÃ³rios'
        });
      }
  ...
  try {
    ...
    if (searchResponse.data.data && searchResponse.data.data.length > 0) {
      ...
      console.log('Resposta da criaÃ§Ã£o:', createResponse.data);
    ...
    console.log('Resposta da criaÃ§Ã£o do pedido:', pedidoResponse.data);
...
async function start() {
  try {
    ...
    console.log('ðŸŸ¢ Servidor BlingBot rodando em http://localhost:3005');
