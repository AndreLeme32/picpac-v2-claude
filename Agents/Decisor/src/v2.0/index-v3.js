...
const criar_proposta = (payload) => {
  try {
    console.log('Iniciando cria̤̣o de proposta para cliente:', payload.nome);
    ...
    if (payload.itens && Array.isArray(payload.itens) && payload.itens.length > 0) {
      itens = payload.itens.map((item) => ({
        descricao: item.descricao || payload.medidas || 'Item sem descri̤̣o',
      ...
      itens = [{
        descricao: payload.medidas || 'Item sem descri̤̣o',
        quantidade: qtd,
...
const decidir = (acao, payload) => {
  console.log('Decisor chamado com a̤̣o:', acao);
  try {
    if (acao === 'criar_proposta') {
      console.log('Executando cria̤̣o de proposta...');
      return criar_proposta(payload);
    } else {
      console.log('A̤̣o ṇo reconhecida:', acao);
      return { success: false, error: `A̤̣o ṇo suportada: ${acao}` };
    }
  } catch (error) {
